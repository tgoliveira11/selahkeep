...
**Vault Status Dock:** on every authenticated page, a small header-attached handle shows vault state (`Vault` when locked, `mm:ss` countdown when open). Expand it for **quick unlock** with vault password or passkey PRF (when configured). Recovery phrase unlock is on **`/vault/unlock`** only. Navigating to `/vault/unlock` keeps the dock collapsed. Authenticated route changes scroll to the top unless the URL includes a hash anchor. `/notes` shows a helpful locked-vault state with **Unlock vault** and **Open full unlock page** when the vault is closed.
