...
/**
 * vault-core 1.0 keeps the session UVK non-extractable, so it cannot be AES-KW
 * wrapped to build a *new* passkey envelope after unlock. We cache the inner
 * vault-key material at unlock (in memory only, cleared on lock) and re-wrap it
 * for the new PRF key when enrolling a passkey. This mirrors vault-core's own
 * `innerVaultKeyBlob` re-wrap path without exporting raw UVK bytes to storage.
 */
...
/** Copy the first 32 PRF bytes for AES-256 import (matches vault-core). */
function prfBytesForAes256(prfOutput: Uint8Array): Uint8Array {
  return prfOutput.byteLength === 32 ? prfOutput : prfOutput.slice(0, 32);
}

async function importPrfAsAesGcmKey(prfOutput: Uint8Array): Promise<CryptoKey> {
  return crypto.subtle.importKey(
    "raw",
    toBufferSource(prfBytesForAes256(prfOutput)),
...
/**
 * Creates a passkey PRF vault-key envelope, reusing cached inner material when the
 * session UVK is non-extractable (vault-core 1.0+ session keys after unlock).
 */
export async function createPasskeyEncryptedVaultKey(
  ...
  try {
    ...
    // Whether the cache was missing or stale (mismatched and discarded), the fix is
    // the same: re-unlock on this device to cache fresh inner material.
