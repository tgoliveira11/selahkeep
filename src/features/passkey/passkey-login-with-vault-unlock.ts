...
export async function signInWithPasskey(
  ...
  const afterLoginPath = options.afterLoginPath ?? "/notes";
  ...
  const credentialId = verifyResult.credentialId ?? assertion.id;
  ...
  const vaultMetadata = await passkeyLoginApi.vaultUnlockMetadata({
    loginToken: verifyResult.loginToken,
    credentialId,
  });

  let encryptedVaultKey = vaultMetadata.encryptedVaultKey;
  let prfRequired = vaultMetadata.prfRequired;

  if (
    vaultMetadata.vaultUnlockAvailable &&
    encryptedVaultKey &&
    !prfOutput &&
  ...
  if (vaultMetadata.vaultUnlockAvailable && encryptedVaultKey && prfOutput) {
    try {
      ...
      logPasskeyLoginVaultEvent("passkey_login_vault_auto_unlock_succeeded", { method: "passkey" });
    } catch (error) {
      if (error instanceof PasskeyPrfRequiredError) {
        sessionStorage.setItem(outcomeKey, "prf-unavailable");
        logPasskeyLoginVaultEvent("passkey_login_vault_auto_unlock_unavailable", {
          method: "passkey",
  ...
  } else if (vaultMetadata.vaultUnlockAvailable && !prfOutput) {
  ...
  } else if (!vaultMetadata.vaultUnlockAvailable) {
    logPasskeyLoginVaultEvent("passkey_login_vault_auto_unlock_unavailable", {
      method: "passkey",
