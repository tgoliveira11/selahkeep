...
describe("@tgoliveira/vault-core dependency", () => {
  it("package.json pins ^1.0.0", () => {
    expect(readJson("package.json").dependencies?.["@tgoliveira/vault-core"]).toBe("^1.0.0");
  });

  it("lockfile resolves 1.0.0", () => {
    const lock = readFileSync(join(ROOT, "package-lock.json"), "utf8");
    expect(lock).toContain('"@tgoliveira/vault-core": "^1.0.0"');
    expect(lock).toMatch(/"node_modules\/@tgoliveira\/vault-core":\s*\{[^}]*"version":\s*"1\.0\.0"/);
  });
