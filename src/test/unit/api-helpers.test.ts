import { describe, it, expect, vi } from "vitest";
import { apiError, parseJsonBody } from "@/lib/api-helpers";
import { PlaintextRejectionError } from "@/server/policies/plaintext-rejection";
import { UnauthorizedError } from "@/lib/auth/session";
import { NotFoundError } from "@/server/services/vault-service";
import { ConflictError, RateLimitError } from "@/server/services/vault-service";
import { AadValidationError } from "@/server/policies/aad-validation";

describe("api helpers", () => {
  it("parseJsonBody returns parsed JSON", async () => {
    const body = await parseJsonBody(
      new Request("http://localhost", {
        method: "POST",
        body: JSON.stringify({ ok: true }),
      })
    );
    expect(body).toEqual({ ok: true });
  });

  it("parseJsonBody returns empty object on invalid JSON", async () => {
    const body = await parseJsonBody(
      new Request("http://localhost", { method: "POST", body: "not-json" })
    );
    expect(body).toEqual({});
  });

  it("apiError maps unauthorized to 401", async () => {
    const res = apiError(new UnauthorizedError("Authentication required"), "GET /test");
    expect(res.status).toBe(401);
    await expect(res.json()).resolves.toEqual({ error: "Authentication required" });
  });

  it("apiError maps plaintext rejection to 400", async () => {
    const res = apiError(new PlaintextRejectionError("title"), "POST /letters");
    expect(res.status).toBe(400);
  });

  it("apiError maps AAD validation errors to 400", async () => {
    const res = apiError(new AadValidationError("bad aad"), "POST /vault");
    expect(res.status).toBe(400);
  });

  it("apiError maps named NotFoundError to 404", async () => {
    const res = apiError(new NotFoundError("missing"), "GET /letters/1");
    expect(res.status).toBe(404);
  });

  it("apiError maps named ConflictError to 409", async () => {
    const res = apiError(new ConflictError("conflict"), "POST /vault");
    expect(res.status).toBe(409);
  });

  it("apiError maps named RateLimitError to 429", async () => {
    const res = apiError(new RateLimitError("slow down"), "POST /vault");
    expect(res.status).toBe(429);
  });

  it("apiError maps named ValidationError to 400", async () => {
    const error = new Error("invalid");
    error.name = "ValidationError";
    const res = apiError(error, "POST /test");
    expect(res.status).toBe(400);
  });

  it("apiError maps unknown errors to 500", async () => {
    const logSpy = vi.spyOn(console, "error").mockImplementation(() => undefined);
    const res = apiError(new Error("boom"), "GET /test");
    expect(res.status).toBe(500);
    logSpy.mockRestore();
  });
});
