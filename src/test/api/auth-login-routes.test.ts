...
vi.mock("@/lib/secure-auth", () => ({
  secureAuth: {
    routes: {
      loginVerify2faOauth: { POST: mocks.verifyOAuthTwoFactor },
      twoFactorBackupCodesRegenerate: { POST: mocks.regenerateBackupCodes },
    },
  },
}));
...
describe("auth login and backup code API routes", () => {
  ...
  it("verify-2fa-oauth returns upgrade token", async () => {
    mocks.verifyOAuthTwoFactor.mockResolvedValue(
      new Response(JSON.stringify({ upgradeToken: "upgrade-token" }), {
        status: 200,
  ...
  it("backup code regeneration returns codes once", async () => {
    mocks.regenerateBackupCodes.mockResolvedValue(
      new Response(JSON.stringify({ backupCodes: ["AAAA-BBBB-CCCC"] }), {
        status: 200,
        headers: { "content-type": "application/json" },
      })
    );
