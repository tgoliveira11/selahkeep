...
3. **Near real-time transcription (bounded streaming).** While recording, the **in-progress segment** (the recent, uncommitted audio) is transcribed on-device on a short interval (~1.2s) and a **live transcript** is shown as it grows. Once a segment passes ~16s (under Whisper's 30s attention window) its text is **committed** and a fresh segment starts, so every partial pass is a single, fast inference over a short window — the per-pass cost stays constant no matter how long the recording is (the old approach re-transcribed the whole, ever-growing buffer, which fell progressively behind). Passes are serialized so the cadence self-throttles to the device's speed. On **Stop**, one accurate pass runs over the **full buffer** to produce the authoritative transcript for review. Nothing is uploaded.
   - **WebGPU first.** The worker probes `navigator.gpu` and loads the pipeline on **WebGPU** (fp32 weights) when available — dramatically faster than CPU — falling back to **WASM** (q8) automatically. Background warm-up also runs one tiny inference on silence so GPU shaders/kernels are compiled before first use. The active backend is surfaced in the UI ("Voice model ready · GPU").
...
The worker lazy-loads the pipeline once (WebGPU when available, else WASM) and caches it across calls, so the incremental passes reuse the same in-memory model. Each **partial** pass transcribes only the recent uncommitted segment; the **final** pass (on Stop) transcribes the full buffer for accuracy. Audio is transferred to the worker via a transferable `ArrayBuffer`; the main thread keeps the source PCM chunks (to slice the next segment and to rebuild the full buffer for the final pass). `env.allowLocalModels=false` fetches weights remotely (self-hostable via `NEXT_PUBLIC_VOICE_MODEL_HOST`, which also serves the ONNX-runtime WASM). Capture uses an **AudioWorklet** served as a static same-origin module (`/worklets/audio-capture-worklet.js`), allowed by the CSP `script-src/worker-src 'self'`; it runs off the main thread for glitch-free capture during inference.
