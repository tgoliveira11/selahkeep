...
Branch: `main` (Phase 0 complete) · **Current pin:** `0.5.0` (see `package.json`)
...
**Complete on `main`.** Account/auth is owned exclusively by `@tgoliveira/secure-auth` (currently `0.5.0`). Guard tests: `no-local-auth-implementation.test.ts`, `secure-auth-delegate-routes.test.ts`, `secure-auth-env-and-imports.test.ts`.
